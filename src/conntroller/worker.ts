...
export class Worker {
  ...
  private async shouldProcessSync(
    ...
    if (subnetBlockNumber < smartContractCommittedHeight) {
      const subnetHashInSmartContract =
        await this.mainnetClient.getBlockHashByNumber(subnetBlockNumber);
  ...
  private async findDivergingPoint(
    ...
    const mainnetHash = await this.mainnetClient.getBlockHashByNumber(
      heightToSearchFrom
    );
